...
class CAL_Dataset(Dataset):
    def __init__(self, root_dir, t, seq_len, subset_len=0):
       ...
       # df_all = torch.tensor(df_all.values)
        ...
        self.mark = df['mark']
        self.gametimeStamp = df['gametimeStamp']
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.seq_len):
            ...
            #imname.append(torch.as_tensor([idx+i]))
            gametime.append(torch.as_tensor([self.gametimeStamp.iloc[idx+i]]))
            #sequenceid.append(torch.as_tensor([self.seqid.iloc[idx+i]]))
            frames.append(im.unsqueeze(0))
            
        inputs['sequence'] = torch.cat(frames)
        inputs['gametimeStamp'] = torch.cat(gametime)
...
def get_data(data_path, seq_len, batch_size):
    ...
    return (
        ...
        DataLoader(val_ds, batch_size=batch_size, pin_memory=False, num_workers=14),
